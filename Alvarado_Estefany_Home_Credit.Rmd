...
# Step 2: Analyzing the target variable and predictor relationships
...
comparison_table %>%
  kable(
    caption = "Comparison of Naive Bayes Models with Various Sampling Methods",
    col.names = c("Model", "F1 Score", "AUC", "Sensitivity", "Precision", "Accuracy"),
    format = "html"
  ) %>%
  kable_styling(
...
comparison_table_2 %>%
  kable(
    caption = "Comparison of Random Forest, XGBoost, and LightGBM Models",
    col.names = c("Model", "Accuracy", "Sensitivity", "Specificity", "Precision", "F1 Score", "AUC"),
    format = "html"
  ) %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "responsive"),
...
comparison_table_3 %>%
  kable(
    caption = "Comparison of Logistic Regression Models with SMOTE and ROSE",
    col.names = c("Model", "F1 Score", "AUC", "Sensitivity", "Precision", "Accuracy"),
    format = "html"
  ) %>%
  kable_styling(
...
# Display top features using kable
importance %>%
  head(10) %>%  # Display only the top 10 features for brevity
  kable(
    caption = "Top 10 Features Based on Importance",
    col.names = c("Feature", "Gain", "Cover", "Frequency"),  # Adjust to match actual column names
    format = "html"
  ) %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "responsive"),
...
comparison_table_5 %>%
  kable(
    caption = "Comparison of LightGBM Models with Metrics",
    col.names = c(
      "Model", "Accuracy", "Sensitivity", "Specificity", 
      "Precision", "F1 Score", "AUC", "Optimal Threshold"
    ),
    format = "html"
  ) %>%
  kable_styling(
...
risk_summary %>%
  kable(
    caption = "Summary of Loans and Default Probabilities by Risk Category",
    col.names = c(
      "Risk Category", "Total Loans", "Average Loan", 
      "Average Default Probability", "Average Income", 
...
```{r}
# Extract and display the 20 best predictors based on importance
importance_predictors <- lgb.importance(lgb_model_3, percentage = TRUE)
top_20_predictors <- importance_predictors[1:20, ]  # Select top 20 predictors

# Display the top 20 predictors using kable
top_20_predictors %>%
  kable(
    caption = "Top 20 Predictors Based on Importance",
    col.names = c("Feature", "Gain", "Cover", "Frequency"),  # Adjust as per the column names in your data
    format = "html"
  ) %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "responsive"),
...
```{r}
# Display personas_table using kable
personas_table %>%
  kable(
    caption = "Personas Table with Key Characteristics",
    col.names = c(
      "Risk Category", "Common Age Group", "Average Loan", "Average Income",
      "Average Repayment Delay", "Expected Interest Revenue",
      "Expected Loss", "Net Profit", "Risk-Adjusted ROI", "Key Characteristics"
    ),
    format = "html"
  ) %>%
  kable_styling(
...
profitability_personas %>%
  ...
  kable(
    caption = "Combined Results of Profitability Personas",
    col.names = c(
      "Risk Category", "Common Age Group", "Average Loan", "Average Income", 
      "Average Repayment Delay", "Expected Interest Revenue", 
      "Expected Loss", "Net Profit", "Risk-Adjusted ROI"
    ),
    format = "html"
  ) %>%
  kable_styling(
